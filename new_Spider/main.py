...
class MainSpider:
    ...
    def loop(self):
        for goods in self.goods:
            ...
            for idx, goods_url in enumerate(self.goods_url):
                if idx == 2:
                    break
                else:
                    self.res = []
                    ...
                    t1 = threading.Thread(target=self.download, args=(goods, self.res, "视频"))
                    t2 = threading.Thread(target=self.download, args=(goods, self.res1, "主图"))
                    t3 = threading.Thread(target=self.download, args=(goods, self.res2, "详情页"))
                    t4 = threading.Thread(target=self.download, args=(goods, self.res3, "SKU"))
                    t1.start()
                    t2.start()
                    t3.start()
                    t4.start()
    ...
    def download(self, goods, res_list, folder_name):
        # 生成文件夹
        save_dir = os.path.join(goods, folder_name)
